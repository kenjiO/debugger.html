...
describe("BreakpointsContextMenu", () => {
  ...
  it("'remove others' calls removeBreakpoints with correct arguments", () => {
    const { props } = render();
    ...
    const removeBreakpointsArg0 = props.removeBreakpoints.mock.calls[0][0];

    const expected = [props.breakpoints[1], props.breakpoints[2]];
    expect(removeBreakpointsArg0).toHaveLength(expected.length);
    expect(removeBreakpointsArg0).toEqual(expect.arrayContaining(expected));
  });

  it("'enable others' calls toggleBreakpoints with correct arguments", () => {
    ...
    const toggleBreakpointsArg0 = props.toggleBreakpoints.mock.calls[0][0];
    expect(toggleBreakpointsArg0).toBe(false);

    const expected = [props.breakpoints[1], props.breakpoints[2]];
    const toggleBreakpointsArg1 = props.toggleBreakpoints.mock.calls[0][1];
    expect(toggleBreakpointsArg1).toHaveLength(expected.length);
    expect(toggleBreakpointsArg1).toEqual(expect.arrayContaining(expected));
  });

  it("'disable others' calls toggleBreakpoints with correct arguments", () => {
    const { props } = render();
    const menuItems = buildMenu.mock.calls[0][0];
    ...
    const toggleBreakpointsArg0 = props.toggleBreakpoints.mock.calls[0][0];
    expect(toggleBreakpointsArg0).toBe(true);
